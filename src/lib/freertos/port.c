...
static void prvSetupTimerInterrupt( void )
{
	...
	#elif (defined TCD1_CCA_vect)  // Xmega Board Timer C0
		#define RT_VECT TCD1_CCA_vect
		TCD1_CCAH = compare_match.byte[1];      // Set high byte of compare match
		TCD1_CCAL = compare_match.byte[0];      // register, then low byte
		TCD1_CTRLA = (1 << TC1_CLKSEL0_bp);		// Prescale F_CPU / 1,
		TCD1_CTRLB = (1 << TC1_CCAEN_bp | 1 << TC1_WGMODE0_bp);		// Enable Compare Capture A
		TCD1_INTCTRLB = (0 << TC1_CCAINTLVL1_bp | 1 << TC1_CCAINTLVL0_bp);
	#else
